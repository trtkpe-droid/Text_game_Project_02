...
player:
  # 戦闘ステータス
  combat_stats:
    sp: 100  # シールドポイント（マジックシールドの耐久値、後述）
    sp_max: 100
...
SP（シールドポイント）はプレイヤーを守る魔法障壁である。

**基本ルール:**
1. **ダメージ吸収**: SPが0より大きい間、敵からのHPダメージはすべてSPで吸収される
2. **拘束攻撃の無効化**: SPが0より大きい間、敵の拘束攻撃（bind_attack）は無効
3. **SP破壊後**: SPが0になると、以降のダメージはHPに直接与えられる
4. **ゲームオーバー**: HPが0以下になるとゲームオーバー

**ダメージ計算例:**
```
SP: 30, HP: 80 の状態で 50ダメージを受けた場合:
→ SPで30を吸収（SP: 0）
→ 残り20ダメージがHPへ（HP: 60）
```

**例外:**
- PT（快楽値）によるダメージはSPを無視してHPに直接影響
- 絶頂（PT >= pt_max）時のHP減少はSPで防げない

---

【烙印システム】

絶頂によるゲームオーバー時、そのモンスターの「烙印」がプレイヤーに刻まれる。

**烙印の効果:**
- 烙印を持つモンスターに対して、プレイヤーの攻撃力にデバフがかかる
- デバフ量は烙印ごとに設定可能（デフォルト: 攻撃力-20%）

**データ構造:**
```yaml
# プレイヤーの烙印リスト
brands:
  - enemy_id: succubus      # 烙印を刻んだ敵のID
    debuff_ratio: 0.2       # 攻撃力20%減
    acquired_at: "2024-01-15"  # 取得日時（任意）

  - enemy_id: slime_queen
    debuff_ratio: 0.15      # 攻撃力15%減
```

**烙印の付与条件:**
1. PTが上限（pt_max）に達して絶頂
2. 絶頂によるHP減少でHPが0以下になりゲームオーバー
3. その時点で戦闘中の敵の烙印が刻まれる

**烙印の解除:**
- 特定のイベントやアイテムで解除可能（MOD側で定義）
- `remove_brand` エフェクトで解除

```yaml
# 烙印解除の例
effects:
  - type: remove_brand
    enemy_id: succubus  # この敵の烙印を解除
```
